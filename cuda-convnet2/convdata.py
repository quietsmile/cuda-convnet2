...
#---------------------
    ...
    def rotate_and_crop_array(self, n_array):
        sz = 256
        ndim = 1
        np_neg_img = 255 - n_array
        neg_img = Image.fromarray(np_neg_img)
        rnd = nr.random() * 360
        neg_rot_img = neg_img.rotate(rnd, Image.BICUBIC);

        if(0):
            np_neg_rot_img = n.array(neg_rot_img, dtype='float32')
            
            tot = np_neg_rot_img.sum()
            sum0 = np_neg_rot_img.sum(0)
            sum1 = np_neg_rot_img.sum(1)

            
            row_idx = n.where(sum1>=0.1)
            lx = row_idx[0][0]
            rx = row_idx[0][-1]
            col_idx = n.where(sum0>=0.1)
            ly = col_idx[0][0]
            ry = col_idx[0][-1]
                   
            #midx= int((lx + rx) / 2.0 + 0.5)
            #midy= int((ly + ry) / 2.0 + 0.5)
            xarray = n.arange(sz).reshape(sz,1) + n.ones(sz) - 1
            yarray = xarray.transpose()

            midx = (xarray * np_neg_rot_img).sum() / (tot)
            midy = (yarray * np_neg_rot_img).sum() / (tot)
            midx = int(midx + 0.5)
            midy = int(midy + 0.5)

            
            
            assert(midx >= lx and midy >= ly and rx >= midx and ry >=midy)
            rect_sz = max(midx - lx, midy - ly, rx - midx, ry -midy) + 2;
            ...
            #neg_rot_img = neg_rot_img.crop((nlx, nly, nrx, nry)).resize((sz, sz))
            neg_rot_img = neg_rot_img.crop((nly, nlx, nry, nrx)).resize((sz, sz))

        ret_array = 255 - n.array(neg_rot_img, dtype='uint8')


        #img = Image.fromarray(ret_array)
        #img.save('test.jpg')
        return ret_array

    def rotate_and_crop(self, n_img):
        sz = 256
        ndim = 1
        ret_array = n.array(n_img)
        for i in range(n_img.shape[3]):
            tmp = n_img[:,:,:,i]
            tmp = tmp.swapaxes(0,2).swapaxes(0,1)
            tmp = tmp.flatten().reshape((sz, sz))
            rot_tmp = self.rotate_and_crop_array(tmp)
            ret_array[:,:,:,i] = rot_tmp.flatten().reshape((ndim, sz, sz))
            #time.sleep(10)
        return ret_array
...
#---------------------
    ...
    def __trim_borders(self, x, target):
        z = x.reshape(self.num_colors, self.img_size, self.img_size, x.shape[1])
        y = self.rotate_and_crop(z) 
